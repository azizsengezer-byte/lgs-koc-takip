...
function renderMobileNav() {
  ...
  nav.innerHTML = `<div class="mobile-nav-inner">${items.map(n=>{
    let badge = '';
    if(n.id==='messages' && unreadMsg>0) badge=`<span style="position:absolute;top:-4px;right:-4px;background:var(--accent2);color:#fff;font-size:0.55rem;font-weight:800;min-width:14px;height:14px;border-radius:99px;display:flex;align-items:center;justify-content:center;padding:0 3px">${unreadMsg>9?'9+':unreadMsg}</span>`;
